  ...
  const fetchCotas = useCallback(async () => {
    try {
      ...
      setCotas((data || []).map(c => ({
        ...
        valor_carro: c.valor_carro != null ? Number(c.valor_carro) : null,
        valor_moto: c.valor_moto != null ? Number(c.valor_moto) : null,
        valor_camionete: c.valor_camionete != null ? Number(c.valor_camionete) : null,
        percentual_geral: Number(c.percentual_geral) || 0,
        percentual_extra: Number(c.percentual_extra) || 0,
        categoria: c.categoria as 'CARRO' | 'MOTO' | 'CAMINHONETE' | undefined,
      })));
    } catch (err) {
